...
begin
    ...
    несортированную части. В самом начале i = 2, и каждый раз i
    увеличивается на единицу, пока не достигнет конца массива: i = N. Цикл
    for подходит для такого случая идеально. }
    for i := 2 to N do
    begin
        { x --- временная переменная, она хранит значение текущего элемента,
        с которой мы сравниваем сортированную колоду }
        x := a[i];
        ...
        каждом шаге, поэтому надо проверить условие что j всё ещё указывает
        на элементы массива. Также, в силу того, что элементы нижней части
        уже сортированы, нам нет смысла проверять их все, нам нужно найти
        ровно то место, куда нужно поместить x. }
        n_comp := n_comp + 1;
        ...
        { В конце проходки по сортированной колоде мы вставляем x на
        найденное место (а это j + 1). }
        a[j + 1] := x;
        n_move := n_move + 1
    end
end;
